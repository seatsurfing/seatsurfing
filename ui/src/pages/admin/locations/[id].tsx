...
class EditLocation extends React.Component<Props, State> {
  ...
  saveSpaces = async () => {
    ...
    for (let item of this.state.spaces) {
      if (item.changed) {
        ...
        space.publicBookingEnabled =
          (item.approvers?.length ?? 0) > 0 && item.publicBookingEnabled;
  ...
  newSpaceState = (e?: Space): SpaceState => {
    const res: SpaceState = {
      ...
      publicBookingEnabled: e ? e.publicBookingEnabled : false,
  ...
  renderRow = (space: SpaceState, rowNumber: number) => {
    ...
    return (
      ...
      >
        ...
        <td>{RendererUtils.state(space.publicBookingEnabled)}</td>
        <td>{bookingLink}</td>
      </tr>
    );
  };
          ...
          >
            ...
            >
              ...
              <Col sm="8">
                <Form.Check
                  type="checkbox"
                  id="space-public-booking-enabled"
                  label={RendererUtils.capitalize(this.props.t("yes"))}
                  checked={this.getSelectedSpace()?.publicBookingEnabled}
                  disabled={
                    (this.getSelectedSpace()?.approvers?.length ?? 0) === 0
                  }
                  onChange={(e: any) => {
                    ...
                    const idx = this.state.selectedSpace!;
                    const space = { ...spaces[idx] };
                    space.publicBookingEnabled = e.target.checked;
                    space.changed = true;
                    spaces[idx] = space;
                    this.setState({ spaces: spaces, changed: true });
                  }}
  ...
  exportTable = (e: any) => {
    ...
    const rows = this.state.spaces.map((space) => [
      ...
      RendererUtils.stateXls(space.publicBookingEnabled, t),
          ...
          >
            <thead>
              <tr>
                ...
                <th>{this.props.t("publicBooking")}</th>
