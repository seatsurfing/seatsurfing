...
const bookingUiUrl = process.env.BOOKING_UI_URL ? process.env.BOOKING_UI_URL : 'http://localhost:8080';
...
test('crud booking', async ({ page }) => {
  ...
  await expect(page.getByRole('dialog').getByText('Book a space')).toBeVisible();
  await page.getByRole('button', { name: 'Confirm booking' }).click();
  await expect(page.getByRole('dialog').getByText('Your booking has been confirmed!')).toBeVisible();
  await page.getByRole('button', { name: 'My bookings' }).click();
  await expect(page).toHaveURL(/bookings\/$/);
  await expect(page.getByText('Loading...')).not.toBeVisible();
  await page.getByText(/Sample Floor/).first().click();
